...
void SPACE::add_eq(double eq_a)//加地震动
{
	int i;
	for(i=0;i<cir.size();i++)
	{
		cir[i].f.x+=cir[i].m*eq_a;
	}
	for(i=0;i<tri.size();i++)
	{
		tri[i].f.x+=tri[i].m*eq_a;
	}
	for(i=0;i<rect.size();i++)
	{
		rect[i].f.x+=rect[i].m*eq_a;
	}
}

void SPACE::init_cmd()
{
	printf("CMD:Group 1:cir,tri,rect,tercir,tertri,terrect(add these elements)\n\tGroup 2:finish\n");
    ...
    while (1)
    {
		...
		if(strcmp(cmd,"cir")==0)
		{
			this->add_element<class Circle>(cir);
		}
		if(strcmp(cmd,"tri")==0)
		{
			this->add_element<class Triangle>(tri);
		}
		if(strcmp(cmd,"rect")==0)
		{
			this->add_element<class Rectangle>(rect);
		}
		if(strcmp(cmd,"plane")==0)
		{
			this->add_element<class Plane>(plane);
		}
		if(strcmp(cmd,"tercir")==0)
		{
			this->add_element_ter<class terCircle>(tercir);
		}
		if(strcmp(cmd,"tertri")==0)
		{
			this->add_element_ter<class terTriangle>(tertri);
		}
		if(strcmp(cmd,"terrect")==0)
		{
			this->add_element_ter<class terRectangle>(terrect);
		}
...
void SPACE::generate_graph()//输出整个SPACE的图形化界面
{
	...
	setaspectratio(1, 1);
	setcolor(GREEN);	
	outtextxy(-WIDTH/2+30,-HEIGHT/2+20,out);
	outtextxy(WIDTH/2-100,-HEIGHT/2+20,count_sec_text);
	clearrectangle(-WIDTH/2+30,-HEIGHT/2+70,-WIDTH/2+270,-HEIGHT/2+170);
	if(eq_count==0)
		getimage(pDstImg,-WIDTH/2+30,-HEIGHT/2+70,240,100);
	putimage(-WIDTH/2+30,-HEIGHT/2+70, pDstImg);
...
void circle_circle_detect_collision(class Circle &cir1,class Circle &cir2)//改变输入物体的受力
{
	...
	if(normal(cir1.xy-cir2.xy)<cir1.r+cir2.r)
	{
		...
		fn1=vector_n*((cir1.youngs_modulus*cir2.youngs_modulus/cir1.r/cir2.r)/(cir1.youngs_modulus/cir1.r+cir2.youngs_modulus/cir2.r)*dep*(cir1.r*cir2.r*0.0005));//压强*面积
		...
		cir1.f_color=cir1.f_color+friction_factor(cir1.material,cir2.material)*fn*unit_fric1;
		cir2.f_color=cir2.f_color+friction_factor(cir1.material,cir2.material)*fn*unit_fric2;
...
template <typename T> void circle_poly_detect_collision(class Circle &cir,T &poly)
{
	...
	for(i=0;i<poly.points.size();i++)
	{
		if(cir.is_inside(poly.points[i]))
		{
			...
			fn1=vector_n*((cir.youngs_modulus*poly.youngs_modulus/cir.r/poly.r)/(cir.youngs_modulus/cir.r+poly.youngs_modulus/poly.r)*dep*(cir.r*poly.r*0.0005));//压强*面积
			...
			cir.f_color=cir.f_color+friction_factor(cir.material,poly.material)*fn*unit_fric1;
			poly.f_color=poly.f_color+friction_factor(cir.material,poly.material)*fn*unit_fric2;
			//cir.f_color+=fn;
			//poly.f_color+=fn;
		}
	}
...
{		
		...
		double a_limit=max(10000,v_relative*v_relative*0.5);//数据基于论文,限制加速度
		XY fn1;//弹力，沿径向（法向量方向）
		fn1=vector_n*((poly1.youngs_modulus*poly2.youngs_modulus/poly1.r/poly2.r)/(poly1.youngs_modulus/poly1.r+poly2.youngs_modulus/poly2.r)*dep*(poly1.r*poly2.r*0.0005));//压强*面积
		...
		poly1.f_color=poly1.f_color+friction_factor(poly1.material,poly2.material)*fn*unit_fric1;
		poly2.f_color=poly2.f_color+friction_factor(poly1.material,poly2.material)*fn*unit_fric2;
		//poly1.f_color+=fn;
		//poly2.f_color+=fn;
}
