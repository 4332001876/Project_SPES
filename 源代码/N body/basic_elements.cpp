...
void Star::copy_var_cir(class Star &cir2)//拷贝所有变量值
{
...
void Star::Display()
{
	...
	fillcircle(magnify_ratio*(xy.x+moveX),magnify_ratio*(xy.y+moveY),5);//y坐标正负应该符合反过来
	

}
int Star::is_inside(XY pt)//判断点是否在图形里，是则返回1
{
	if(normal(pt-xy)<=r)
		return 1;
	else
